...
class lotoCard:
    def __init__(self, name):
        bag = [x for x in range(1, 91)]  # Номера в мешке с бочками.
        self.lotocard = [__class__.gen_string(bag), __class__.gen_string(bag),
                     __class__.gen_string(bag)]
        self.name = name
        self.count_barrel = 15  # осталось номеров на карточке
    ...
    def __str__(self):
        ...
        for x in range(3):
            rez += '{:>2} {:>2} {:>2} {:>2} {:>2} {:>2} {:>2} {:>2} {:>2}'\
                    .format(*self.lotocard[x]) + '\n'
        return rez + '--------------------------'

computer = lotoCard(' Карточка компьютера ')
player = lotoCard(' Карточка игрока ')
...
while True:
    ...
    if reply == 'q':
        ...
        for x in range(3):
            if barrel in player.lotocard[x]:
                test = True
                player.lotocard[x][player.lotocard[x].index(barrel)] = '-'
                player.count_barrel -= 1
            if barrel in computer.lotocard[x]:
                computer.lotocard[x][computer.lotocard[x].index(barrel)] = '-'
